...
/**
 * @file
 * Contains \Drupal\scf_mcsi\Form\mcsi_first_alert_form.
 */
...
class mcsi_first_alert_form extends ConfigFormBase {
  
  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'mcsi_first_alert_form';
  }
  /**
   * {@inheritdoc}
   */
  ...
  {
      return ['mcsi_first_alert.settings'];
  }
  
  public function buildForm(array $form, FormStateInterface $form_state) {
   
    $config = $this->config('mcsi_first_alert.settings');
    ...
    $query->condition('r.first_alert', 1);
    ...
    if (empty($tempstore->get('num_rows_mcsi_first_alert_page')) || $tempstore->get('num_rows_mcsi_first_alert_page') == null) {
      $tempstore->set('num_rows_mcsi_first_alert_page', $count_data);

    }
    $form['mcsi_first_alert_title'] = array(
      '#type' => 'textfield',
      '#title' => t('Title'),
      '#default_value' =>  $config->get('mcsi_first_alert_title'),
      '#weight' => -1,
    );
    $form['mcsi_first_alert_items']['#tree'] = TRUE;

    for ($i = 0; $i < $tempstore->get('num_rows_mcsi_first_alert_page'); $i++) {
      $first_alert_id = isset($data[$i]) ? $data[$i]['id'] : 0;
      $remove_path = '/admin/mcsi/core-team/first-alert/experts-remove/'.$first_alert_id;
      
      $remove_link = 'Remove';
      $links = '';
      $links = '<div class="mcsi-first-alert-remove"><a href="'.$remove_path.'">'.$remove_link. '</a></div>';
      $fname = !empty($data[$i]['title']) ? $data[$i]['title'] : '';
      $lname = !empty($data[$i]['field_last_name_value']) ? $data[$i]['field_last_name_value'] : '';
      
      $form['mcsi_first_alert_items'][$i] = [
        'item' => [
          '#type' => 'textfield',
          '#default_value' => !empty($data[$i]['nid']) ? $fname . ' ' .' '. $lname.' [ '.'nid:'.$data[$i]['nid'].' ] ' : '',
          '#size' => 60,
          '#maxlength' => 255,
          '#autocomplete_route_name' => 'scf_mcsi.first_alert_autocomplete',
          '#required' => FALSE,
    ...
    $form['names_fieldset']['actions']['add_name'] = [
      ...
      '#submit' => ['::scf_mcsi_first_alert_form_add_more'],
  ...
  function scf_mcsi_first_alert_form_add_more(array &$form, FormStateInterface $form_state)
  { 
   
    $tempstore = \Drupal::service('tempstore.private')->get('scf_mcsi');
    if (empty($tempstore->get('num_rows_mcsi_first_alert_page')) || $tempstore->get('num_rows_mcsi_first_alert_page') == null) {
      $tempstore->set('num_rows_mcsi_first_alert_page', 1);
    }else{
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->config('mcsi_first_alert.settings')
      ->set('mcsi_first_alert_title', $val['mcsi_first_alert_title'])->save();

    foreach($val['mcsi_first_alert_items'] as $key => $item) {
      ...
      if(!empty($item['item']) && !empty($nid)) {
        $items[] = ['item' => $nid,'weight' =>  $item['weight'],'mcsi_first_alert_id' =>  $item['mcsi_first_alert_id']];     
      }
    }
    $tempstore = \Drupal::service('tempstore.private')->get('scf_mcsi');
    if ($items != null) {
      $tempstore->set('num_rows_mcsi_first_alert_page', count($items));
    }
    // dd($items);
    foreach($items as $key => $item) {
      if($item['mcsi_first_alert_id'] > 0 ){
      ...
      ->condition('id', $item['mcsi_first_alert_id'])
      ->execute();
